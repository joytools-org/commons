 ...
 * @author AndreaR
 */
class NakedBeanAccessorV1 extends NakedMultiValueAccessor<String, Object> {

    protected NakedBeanAccessorV1(final Object bean) throws IntrospectionException {
        this(bean, null);
    }

    protected NakedBeanAccessorV1(final Object bean,
        ...
        for (final var pd : Introspector.getBeanInfo(bean.getClass()).getPropertyDescriptors()) {
            if (!pd.getName().equalsIgnoreCase("class")) {
                ...
                if (read != null && read.canAccess(bean)) {
                    mut.insert(pd.getName(), Tuple(
                            read, 
                            write != null && write.canAccess(bean) ? write : null));
                }
            }
        }
    }
