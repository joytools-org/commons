 ...
 * @author AndreaR
 */
public class BackgroundSuppliers {
    ...
    public static <T> Supplier<T> of(final Supplier<T> supplier) {
        return toSupplier(BackgroundInitializers.of(toEither(supplier)));
    }
    ...
    public static <T> Supplier<T> ofChecked(final Callable<T> callable) {
        return toSupplier(BackgroundInitializers.ofChecked(toEither(callable)));
    }
    ...
    public static <T, E extends Exception> Supplier<T> ofFailable(final FailableSupplier<T, E> failableSupplier) {
        return toSupplier(BackgroundInitializers.ofFailable(toEither(failableSupplier)));
    }
