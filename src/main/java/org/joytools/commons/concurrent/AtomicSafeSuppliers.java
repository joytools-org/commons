 ...
 * @author AndreaR
 */
public class AtomicSafeSuppliers {
    ...
    public static <T> Supplier<T> of(final Supplier<T> supplier) {
        return toSupplier(AtomicSafeInitializers.of(toEither(supplier)));
    }
    ...
    public static <T> Supplier<T> ofChecked(final Callable<T> callable) {
        return toSupplier(AtomicSafeInitializers.ofChecked(toEither(callable)));
    }
    ...
    public static <T, E extends Exception> Supplier<T> ofFailable(final FailableSupplier<T, E> failableSupplier) {
        return toSupplier(AtomicSafeInitializers.ofFailable(toEither(failableSupplier)));
    }
