/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package org.joytools.commons.sql;

import java.sql.ParameterMetaData;
import java.util.HashSet;
import java.util.Set;
import org.joytools.commons.collections.IterableAccessors;
// import org.joytools.commons.collections.StreamIterables;
import org.joytools.commons.lang.CaseSpaceNormalizer;
import org.joytools.commons.lang.EnumLookup;


/**
 *
 * @author AndreaR
 */
public enum ParameterMode {

    /**
     * The constant indicating that the mode of the parameter is unknown.
     */
    UNKNOWN(ParameterMetaData.parameterModeUnknown),

    /**
     * The constant indicating that the parameter's mode is IN.
     */
    IN(ParameterMetaData.parameterModeIn),

    /**
     * The constant indicating that the parameter's mode is INOUT.
     */
    INOUT(ParameterMetaData.parameterModeInOut),

    /**
     * The constant indicating that the parameter's mode is  OUT.
     */
    OUT(ParameterMetaData.parameterModeOut);

    /**
     *
     * @param value
     * @param names
     */
    ParameterMode(final int value,
            final String... names) {
        this.value = value;
        this.names = names;
    }

    /**
     *
     * @param name
     * @return
     */
    public static ParameterMode of(final String name) {
        return LOOKUP.getExisting(name);
    }

    /**
     *
     * @param value
     * @return
     */
    public static ParameterMode of(final int value) {
        for (final ParameterMode t : values()) {
            if (t.value == value) {
                return t;
            }
        }
        throw new IllegalArgumentException("No parameter mode found with value: " + value);
    }

    /**
     *
     * @return
     */
    public int value() {
        return value;
    }

    /**
     *
     */
    private final static EnumLookup<String, ParameterMode> LOOKUP = new EnumLookup<String, ParameterMode>(
            values(),
            CaseSpaceNormalizer.UPPERCASE_TRIM.castAsString()) {

        @Override
        protected Iterable<String> keysOf(final ParameterMode item) {
            final Set<String> keys = new HashSet();
            for (final String n : IterableAccessors.from(item.name()).append(item.names)) {
                keys.addAll(expand(n, "MODE_" + n));
            }
            return keys;
        }

    };

    /**
     *
     */
    private final int value;

    /**
     *
     */
    private final String[] names;

}
